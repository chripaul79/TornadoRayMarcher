...
Mesh generateCylinderMesh(float radius, float height, unsigned int segments) {
    Mesh mesh;
    ...
    for (unsigned int i = 0; i <= segments; ++i) {
        ...
        // Positions
        mesh.vertices.push_back({ x * radius, -halfHeight, z * radius }); // Bottom
        mesh.vertices.push_back({ x * radius,  halfHeight, z * radius }); // Top

        // Normals (pointing straight out from the center axis)
        mesh.normals.push_back({ x, 0.0f, z });
        mesh.normals.push_back({ x, 0.0f, z });

        // UVs
        mesh.textureCoordinates.push_back({ ratio, 0.0f });
        mesh.textureCoordinates.push_back({ ratio, 1.0f });
    }
    ...
    // Note: To add Caps (Top/Bottom), you would need to add 
    // a center vertex for each and a ring of vertices with 
    // normals pointing Up (0, 1, 0) and Down (0, -1, 0).
